...
This message is sent whenever the relayer receives a new order, or when the `remainingFillableTakerAmount` of an order changes. The scenarios where an update is necessary include:
* The relayer received a new order.
* An order was fully or partially filled, so `remainingFillableTakerAmount` has changed.
* The maker of the order no longer has sufficient tokens to fulfill the order, so `remainingFillableTakerAmount` changes to the maximum amount that is fillable by the maker. 
* The order is cancelled, so `remainingFillableTakerAmount` is 0. 

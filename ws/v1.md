...
This message is sent whenever the relayer receives or hears about new orders, or when the `remainingFillableAmount` of an order changes because a fill occurred, or because the order was cancelled (in which case the remaining fillable amount would be 0). Updates can be sent in bulk since the payload of the message specifies a collection of updated or new orders. 

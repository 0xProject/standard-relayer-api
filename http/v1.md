...
### Network Id
All requests should be able to specify a **?networkId** query param for all supported networks. If a certain network is not supported, the response should **400 Bad Request – Invalid request format: Network id {NETWORK_ID} is not supported** .
...
[See error response schema](https://github.com/0xProject/0x-monorepo/blob/development/packages/json-schemas/schemas/relayer_api_error_response_schema.ts#L1)

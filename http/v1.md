...
All endpoints that are paginated should return a **total** and a **records** value in the top level of the response.  The value of **total** should be the total number of records for a given query, whereas **records** should be an array representing the response to the query for that page.

These requests include the `asset_pairs` and `orders` endpoints.
...
Retrieves a list of orders given query parameters. This endpoint should be [paginated](#pagination). For querying an entire orderbook snapshot, the [orderbook endpoint](#get-v1orderbook) is recommended.
